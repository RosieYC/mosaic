...
from utils.general import user_config_dir,is_ascii, xyxy2xywh, is_chinese, xywh2xyxy
...
def check_font(font='Dengl.ttf', size=10):
...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font='Dengl.ttf', pil=False, example='abc'):
        assert im.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to Annotator() input images.'
        self.pil = pil or not is_ascii(example) or is_chinese(example)
        if self.pil:  # use PIL
            self.im = im if isinstance(im, Image.Image) else Image.fromarray(im)
            self.draw = ImageDraw.Draw(self.im)
            self.font = check_font(font='Dengl.Unicode.ttf' if is_chinese(example) else font,
